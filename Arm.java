...
public class Arm
{
   ...
   public void directKinematic(){
       ...
       if (d<2*r){
        ...
        double  h = Math.sqrt(r*r - (yj1-ya)*(yj1-ya) - (xa-xj1)*(xa-xj1));
        double alpha= Math.atan(0.5*(yj1-yj1)/0.5*(xj2-xj1));
         // tool position
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        double h1 = Math.sqrt(dx1*dx1 + dy1*dy1);
        if (h1>2*r){
            UI.println("Arm 1 - can not reach");
           valid_state = false;
            return;
       }
        double ay1 = ym1 + (dy1)/2;
        double ax1 = xm1 + (dx1)/2;
        double l1 = h1/2;
        double d1 = Math.sqrt(r*r - l1*l1);
        // elbows positions
        double alpha1 = Math.PI/2-(Math.PI - Math.atan2(dy1,dx1));
        double xd1 = d1*Math.cos(alpha1);
        double yd1 = d1*Math.sin(alpha1);
        xj1 = ax1 + xd1;
        yj1 = ay1 + yd1;
        ...
        double dx2 = xt - xm2; 
        double dy2 = yt - ym2;
        double h2 = Math.sqrt(dx2*dx2 + dy2*dy2);
        if (h2>2*r){
        ...
        double l2 = h2/2;
        double d2 = Math.sqrt(r*r - l2*l2);
        double alpha2 = Math.PI/2 - (Math.PI - Math.atan2(dy2,dx2));
        double xd2 = d2*Math.cos(alpha2);
        double yd2 = d2*Math.sin(alpha2);
        // elbows positions
        double ay2 = ym2 + (dy2)/2;
        double ax2 = xm2 + (dx2)/2;
        ...
        double error_angle = Math.tan(l1/d1)*2;
        if(error_angle<=Math.PI/4 && error_angle>0){
            valid_state = false;
    ...
    public int get_pwm1(){
        ...
        int pwm = (int) (pwma+ (theta1*(180/Math.PI) - thetaB)*((pwma-pwmb)/(thetaA-thetaB)));
        UI.println("this is pwm 1 " + pwm);
        return pwm;
    }
    // ditto for motor 2
    public int get_pwm2(){   
        ...
        int pwm = (int) (pwma + (theta2*(180/Math.PI) - thetaB)*((pwma-pwmb)/(thetaA-thetaB)));
        UI.println("this is pwm 2 " + pwm);    
        return pwm;
    }
